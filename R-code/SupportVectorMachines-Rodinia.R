...
for( kernelApp in apps[1:8]) {
    ...
    for(gpu in c(1:3, 5:6, 8:10)) {
        ...
        # trainingSet <- apply(trainingSet,2,log)
        # testSet <- log(testSet,2)
        
        base <- svm(trainingSet$duration ~ ., data = trainingSet, kernel="linear", scale=FALSE) 
...
ggsave(paste("./images/SVM-Rodinia.png", sep = ""), Graph, height = 20, width = 20)
