...
kernel <- names(kernelsDict[3])
gpu <- 2

for(kernel in 3:8){
  for(gpu in 2){#NoGPU){  
    temp <- data.frame(read.csv(paste("./datasets/", names(kernelsDict[kernel]), "-", gpus[gpu,"gpu_name"], ".csv", sep = "")))
    tempNvprof <- temp[, which(names(temp) %in% c("duration", "input.size.1", "input.size.2"))]
    
    temp <- read.csv(paste("./data/", kernelsDict[kernel], "-GPU.csv", sep = ""), header = FALSE,stringsAsFactors = FALSE)
    ...
    if (kernelsDict[kernel] == "backprop"){
      ...
      # names(data) <- c("P1") 
      trainingData <- data[data$input.size.1 <= 27648, -c(1,2)][-1,]
      testingData <- data[data$input.size.1 >= 27648,-c(1,2)][-1,]
    }
    ...
    if (kernelsDict[kernel] == "calculate_temp"){
      # data <- data.frame(tempNvprof, temp[,3:(dim(temp)[2]-2)])
      trainingData <- data[data$input.size.1 <= 256 & data$input.size.2 == 256, -c(1,2)][-1,]
      testingData <-  data[data$input.size.1 >= 256 & data$input.size.2 != 256, -c(1,2)][-1,]
    }
    if (kernelsDict[kernel] == "hotspotOpt1"){
      # temp$V3 <- as.numeric(temp$V3)
      # data <- data.frame(tempNvprof, sapply(temp[,3:(dim(temp)[2]-2)], normalizeMax))
      trainingData <- data[data$input.size.1 <= 4 & data$input.size.2 <= 100, -c(1,2)][-1,]
      testingData <- data[data$input.size.1 > 4 & data$input.size.2 > 100,-c(1,2)][-1,]
    }
    if (kernelsDict[kernel] == "needle_cuda_shared_1" | kernelsDict[kernel] == "needle_cuda_shared_2"){
      # temp$V3 <- as.numeric(temp$V3)
      # data <- data.frame(tempNvprof, temp[,3:(dim(temp)[2]-2)])
      # data <- data.frame(tempNvprof, sapply(temp[,3:(dim(temp)[2]-2)], normalizeMax))
      
      trainingData <- data[data$input.size.1 <= 1024 & data$input.size.2 <= 4, -c(1,2)][-1,]
      testingData <- data[data$input.size.1 == 4096 & data$input.size.2 > 4,-c(1,2)][-1,]
    }
    
   predictKernelsLM(trainingData, testingData)
...
predictKernelsLM <- function(trainingData, testingData){
  
  counter <- createCounter(0)
  rss <- matrix(nrow = 12, ncol = 3)
  png(filename = paste("./images/fitModels/", names(kernelsDict[kernel]), ".png", sep=""), width = 1600, height = 800)
  ...
  for(resultY in c("","log2")){
    for(FeaturesX in c("", "log", "exp", "poly")){
      ...
      fit <- lm(as.formula(
          paste(resultY, "(", names(trainFeatures[i]),  ifelse(resultY == "log2", "+ 0.0000000001", ""),") ~",
                paste(FeaturesX, "(",ifelse(FeaturesX == "exp", "normalizeMax(",""),  
                      colnames(trainFeatures[!names(trainFeatures) == i]), ifelse(FeaturesX == "poly", ",3,raw = TRUE)",
                                                                                  ifelse(FeaturesX == "log2", "+ 0.0000000001)", ")")),ifelse(FeaturesX == "exp", ")", "") , 
                      collapse = " + " ))), data =  trainFeatures)
      
      rss[count,1] <- summary(fit)$adj.r.squared
      rss[count,2] <- resultY
      ...
      qqnorm(base, ylab="Studentized Residual",
             xlab="t Quantiles",
             main=paste(" Y=", resultY, " X=", FeaturesX, sep=""),
  ...
  timeKernel <- predict(lm(as.formula(paste(rss[key, 2], "(", names(trainFeatures[i]),  ") ~",
                                                  paste(rss[key,3], "(",ifelse(rss[key,3] == "exp", "normalizeMax(",""),  
                                                        colnames(trainFeatures[!names(trainFeatures) == i]), ifelse(FeaturesX == "poly", ",3,raw = TRUE)",
                                                                                                                    ifelse(rss[key,3] == "log", "+ 0.0000000001)", ")")),ifelse(rss[key,3] == "exp", ")", "") , 
                                                        collapse = " + " ))
  ), data =  trainFeatures), testFeatures)
  
  # timeKernel <- predict(randomForest(names(trainFeatures[i])~., data =  trainingData), testingData[,-1])
  
  if (rss[key, 2] == "log2")
      timeKernel <- 2^(timeKernel) - 0.0000000001 
  
  
  png(filename = paste("./images/fakeFeatures/", names(kernelsDict[kernel]), ".png", sep=""), width = 1600, height = 800)
    par(mfrow=c(1,2))
  plot(testingData[,1], ylim = c(0, max(timeKernel,testingData[,1])),  col="blue", ylab = "Time ms", cex=2,
       main=paste(names(kernelsDict[kernel]), "|  ", rss[key,2], "(y) = ", rss[key,3], "(x)"))
  legend("bottomleft", legend = c("Measured", "Predicted"), col = c("blue", "red"),lwd = 2, cex = 2) 
    points(timeKernel, col="red")
    boxplot(testingData[,1]/timeKernel, ylab="Accuracy. Measured/Predicted", cex=2)
