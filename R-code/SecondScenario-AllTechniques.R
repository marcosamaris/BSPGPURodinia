...
result_Global <- rbind(Result_AM, Result_LM)
...
result_Global <- subset(result_Global, Apps == c("MMGC", "MMSU", "MAU","MSA")) 
...
Graph <- ggplot(data=result_Global, aes(x=Gpus, y=accuracy, group=Gpus, col=Gpus)) + 
    ...
    theme(legend.text  = element_text(family = "Times", face="bold", size=27)) +
    ...
    facet_grid(Technique~Apps, scales="fixed") +
    theme(strip.text = element_text(size=25, family = "Times", face="bold"))

ggsave(paste("./images/ResultTechniques.pdf",sep=""), Graph, device = cairo_pdf, height=10, width=20)
